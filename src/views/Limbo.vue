...
import { randomString, genRanHex, calcBetByMulti, wordSeparator, calcPayoutByPercentage, grabFromLocalStorage, createDefaultChartTheme, setThemeValues } from "@/helpers/helpers"
import { checkParams, updateBet } from "@/helpers/limboHelper"
import Chart from 'chart.js/auto'
import useEventsBus from "@/composables/eventBus"
import { LimboSimulator } from "@/models/limboSimulator"
...
const resetState = () => {
  ...
  limboData.betAmount = [];
  limboData.wagered = 0;
  limboData.highestBet = 0;
  limboData.losses = 0;
  limboData.wins = 0;
  limboData.maxWinStreak = 0;
  limboData.maxLoosingStreak = 0;
  limboData.highestLost = 0;
  limboData.highestSingleWin = 0;
  limboData.highestBalance = 0;
  limboData.lowestBalance = 0;
  balance.value = initialBalance.value;
  currentBet.value = baseBet.value;
  currentTarget.value = baseTarget.value;
  nonce.value = startingNonce.value;
}

const doBet = async () => {
  if(!checkParams(baseBet.value, baseTarget.value)) {
    emitBus('notify', {
      type: 'error',
      title: 'Error betting',
      message: 'Please set a correct BaseBet and baseTarget.',
      action: 'retry'
    });
    return false;
  }
  ...
  info.data.baseTarget = baseTarget.value;
  info.data.currentTarget = currentTarget.value;
  ...
  limboData.balance.push(info.data.balance);
  limboData.betAmount.push(info.data.currentBet);
  ...
  setTimeout(async function () {
    for (let i = startingNonce.value; i < endingNonce.value; i++) {
      ...
      // Check conditions
      if (info.data.exit && displayChart.value) {
        chartData.labels.push(info.data.nonce);
        limboData.balance.push(info.data.balance);
        limboData.betAmount.push(info.data.currentBet);
        break;
...
const updateSetup = (setup) => {
  ...
  baseTarget.value = setup.baseTarget;
