    ...
    <div class="notificationSoundBox">
        <div class="notificationBox">
            ...
            <div class="notificationItem">
                <label for="payoutTargetInput">Select payout:</label>
                <input
                    id="payoutTargetInput"
                    type="number"
                    min="1.01"
                    max="100000"
                    v-model="payoutTarget"
        ...
        <div class="notificationBox">
            ...
            <div class="notificationItem">
                <label for="B2BTargetInput">Select B2B Payout:</label>
                <input
                    id="B2BTargetInput"
                    type="number"
                    min="1.01"
...
const       payoutTarget = ref(2),
    ...
    isPayoutNotification = ref(false),
       ...
       notificationConditions = ref([
        ...
        selectedSound = ref(notificationSounds.value[0]);

let selectedSoundAudio = new Audio(`./sounds/${selectedSound.value}.mp3`);
...
const checkLogic = () => {
    if (isPayoutNotification.value) {
        if (props.targets[props.targets.length - 1] >= payoutTarget.value) {
            playSound();
        }
    }
...
/* MOUNTED */
onMounted(() => {
    const localSound = JSON.parse(localStorage.getItem("notificationSound"));
    if (localSound) {
        selectedSound.value = localSound;
    }
});

/* WATCHER */
watch(payoutTarget, (newValue, oldValue) => {
...
watch(selectedSound, (newValue, oldValue) => {
    if (selectedSound.value) {
        ...
        selectedSoundAudio = new Audio(`./sounds/${selectedSound.value}.mp3`);
    }
});
</script>